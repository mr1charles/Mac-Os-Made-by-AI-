(() => {
  ...
  const appDefs = [
    { id: 'finder', label: 'Finder', icon: '📁', open: () => ({ title: 'Finder', content: FinderApp.finderView() }) },
    { id: 'browser', label: 'Browser', icon: '🌐', open: () => ({ title: 'Browser', content: '<iframe src="apps/browser.html"></iframe>' }) },
    { id: 'mp4', label: 'MP4 Player', icon: '🎬', open: () => ({ title: 'MP4 Player', content: MediaApps.createVideoPlayer() }) },
    { id: 'mp3', label: 'MP3 Player', icon: '🎵', open: () => ({ title: 'MP3 Player', content: MediaApps.createAudioPlayer() }) }
  ];
  ...
  function launch(app) {
    const cfg = app.open();
    WindowManager.openApp({ id: app.id, ...cfg });
